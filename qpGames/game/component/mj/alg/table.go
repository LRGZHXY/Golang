...
// generateKey  []int{2, 0, 1, 0, 0, 3, 0, 0, 1} -->  "201003001"
...
// toNumberArray "201003001" --> []int{2, 0, 1, 0, 0, 3, 0, 0, 1}
...
func (t *Table) tryAdd(cards []int, guiCount int, feng bool) bool {
	for i := 0; i < 9; i++ { //检查牌数量合法性，每张牌的数量必须在0~4之间
	...
	if feng {
		...
		if ok { // 已经存在，说明重复，不添加，返回false
			return false
		}
		t.keyFengGuiDic[guiCount][key] = true //加入字典
		return true
	}
	// 非feng牌
